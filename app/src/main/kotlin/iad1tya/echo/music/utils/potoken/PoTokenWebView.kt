...
class PoTokenWebView private constructor(
    ...
    init {
        ...
        webViewSettings.domStorageEnabled = true // BotGuard may use localStorage for signals
        // Enable hardware acceleration so WebGL and Canvas 2D work at full quality.
        // BotGuard collects GPU/canvas fingerprints — without hardware rendering these are
        // omitted, resulting in shorter (~88-byte) tokens that YouTube rejects for TVHTML5.
        @Suppress("WrongThread") // WebView init already runs on MainThread via PoTokenWebView
    ...
    fun onRunBotguardResult(botguardResponse: String) {
        Timber.tag(TAG).d("botguardResponse received")
        makeBotguardServiceRequest(
    ...
    @JavascriptInterface
    fun onMinterCreated() {
        Timber.tag(TAG).d("poToken minter created successfully")
    ...
    suspend fun generatePoToken(identifier: String): String {
        return withContext(Dispatchers.Main) {
            suspendCancellableCoroutine { cont ->
                Timber.tag(TAG).d("generatePoToken() called: $identifier")
    ...
    fun onObtainPoTokenError(identifier: String, error: String) {
        Timber.tag(TAG).e("obtainPoToken error: $error")
    ...
    fun onObtainPoTokenResult(identifier: String, poTokenU8: String) {
        ...
        Timber.tag(TAG).d("Generated poToken for: $identifier")
